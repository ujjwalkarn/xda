...
bivariate<-function(df,dep.var,indep.var,n.bins=4,na.rm=TRUE){
  ...
  if((class_var[[1]] %in% c("integer","numeric")) & 
    ...
    df.result          <-NULL
    c.indep.level      <- levels(df[,c.indep.var])
    c.dep.level        <- levels(df[,c.dep.var])
    ...
    colnames(df.result) <- c(c.indep.var,c.dep.level)
    ...
    colnames(df.result) <- c(paste0("bin_",c.indep.var),c.dep.level)
